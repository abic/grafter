...
module Grafter
  class Debootstrap
    ...
    def install
      Commands::Debootstrap.new(suite, target, mirror).run
      update_source_list
      reconfigure_locale
    ...
    def reconfigure_locale
      Chroot.new(target).use do |chroot|
        chroot.run Commands::LocaleGen.new('en_US.UTF-8')
        chroot.run Commands::UpdateLocale.new('LANG=en_US.UTF-8')
        chroot.run Commands::DpkgReconfigure.new('libc6')
        chroot.run Commands::DpkgReconfigure.new('locales')
      end
    end
    ...
    def reconfigure_timezone
      ...
      Chroot.new(target).use do |chroot|
        chroot.run Commands::DpkgReconfigure.new('tzdata')
      end
    end
